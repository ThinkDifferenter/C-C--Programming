...
// 	} 
	...
	for (j=0; j<m_Dim; j++)
	{
		if(Node[j].m_Averge)
		{
			...
			{
				T_weight=Node[j].m_Weight;
				T_avg=Node[j].m_Averge;
				break;
			}			
		} 
	}
	...
	for(j=0;j<m_Dim;j++)
	{
		if(Node[j].m_Averge==T_avg)
		{
			printf("平均价值为%.2f的物件价值:%.2f,体积:%.2f\n",T_avg,Node[j].m_Profit,Node[j].m_Weight);	
		}
	}
//	for (j=0; j<m_Dim; j++)
//	{
...
//			Sum_weight+=Node[j].m_Weight;
//			Node[j].flag=1;
//			if(Sum_weight>=m_Contain-T_weight)
//			{
//				printf("\n体积超限！\n"); 
//				break;
//			}			
